...
void disableSynth() {
	...
	OCR1B = 0; // Set PWM duty cycle to 0%
...
volatile uint8_t waveType = SINE;
...
ISR(TIMER0_COMPA_vect) {
	...
	for (int i = 0; i < MAX_FREQS; i++) {
		...
		if (waveType == SINE) {
			...
			sample += sineLUT[accum8Bit>>1];

		} else if (waveType == TRIANGLE) {
			if (accum8Bit < 128) {
				sample += accum8Bit<<1; // Increase sample for first half
			} else {
				sample += (255-accum8Bit)<<1; // Decrease sample for second half
			}
